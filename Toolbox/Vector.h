...
namespace Toolbox {
	...
	public:
		...
		/** Returns a pointer to the element after the last element in the vector. */
		Type* end() {
			return data + size;
		}

		/** Returns a constant pointer to the element after the last element in the vector. */
		...
		Type& Emplace(const size_t Index, ArgumentTypes&&... Arguments) {
			...
			return data[Index] = Type(std::forward<ArgumentTypes>(Arguments)...);
		}
